...
(* The measurement of step 4 of docs/improviser.md: does the texture of the endless walk
   hold? One number over a whole draw cannot answer it, because a good opening hides a bad
   end. Therefore each seed gives a row of windows, and the report quotes the spread over
   the seeds window by window: the walk holds when the last window reads like the first
   and like the corpus. *)
let texture ~label ~span ~music =
  let rows = List.map music ~f:(Texture.windows ~span) in
  let reference =
    Texture.windows
      (Texture.steps_of_codes (Jsb.pack (Jsb.load ~path:Jsb.default_path).train).codes)
      ~span:Int.max_value
  in
