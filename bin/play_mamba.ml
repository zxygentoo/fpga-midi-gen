(* play_mamba: samples the step-frame model, prints the steps, and with -play sends them
   to the synthesizer.
...
let command =
  Command.basic
    ~summary:"sample the state-space checkpoint; print the steps, or play them"
    (let%map_open.Command checkpoint =
     ...
     and min_p =
       flag
         "-min-p"
         (optional_with_default Mamba.elected_min_p float)
         ~doc:
