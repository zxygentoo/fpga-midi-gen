...
`fixed.py` holds the part of the integer arithmetic that is one thing across the eras, and
`lib/nn/quantized.ml` holds the same rules in OCaml, where the elaborations read them. The
two are TWO STATEMENTS OF ONE RULE and nothing in the types welds them: what stands here
is the arithmetic, stated in numbers, that both must give.

THESE ARE THE SMALLEST GATES OF A TWIN. `test_rtl.py` holds a twin against its circuit,
...
def test_the_exponent_rule_holds_at_its_boundaries(peak, exponent):
    ...
    assert fixed.max_exponent(peak) == exponent
...
def test_the_byte_is_symmetric_and_ties_round_up():
    ...
    q, e = fixed.quantize(np.array([0.02, -0.01, 0.0]))
    assert (list(q), e) == ([82, -41, 0], 12)
    assert fixed.round_half_up([-5.5, -2.5, 2.5, 5.5]).tolist() == [-5, -2, 3, 6]
...
def test_a_stated_exponent_overrides_the_tensors_own_peak():
    ...
    assert list(fixed.quantize(np.array([0.02, -0.01, 0.0]), e=10)[0]) == [20, -10, 0]
    assert list(fixed.quantize(np.array([1.0, -1.0]), e=14)[0]) == [127, -127]
...
def test_the_sigmoid_table_is_the_shared_table():
    ...
    assert fixed.sigmoid_q(np.int64(0)) == 16640
    assert fixed.silu(np.int64(0)) == 0
...
def test_the_softplus_table_is_the_correction_alone():
    ...
    assert fixed.softplus(np.int64(4096 * 7)) == (4096 * 7) + table[224]
    # the sum rides an int16, thus the result clamps
    assert fixed.softplus(np.int64(32767)) == fixed.INT16_HIGH
    # the clamp of the index catches the one value whose magnitude does not fit the table
    assert fixed.softplus(np.int64(-32768)) == table[255]
...
def test_the_temper_is_log2e_over_the_temperature():
    ...
    assert fixed.temper_of(1.0) == (23637, 14)
    assert fixed.temper_of(0.5) == (47274, 14)
...
def test_the_min_p_floor_is_a_share_of_the_peak_weight():
    ...
    assert fixed.min_weight_of(0.05) == 1638
    assert fixed.min_weight_of(0.0) == 0
    for outside in (-0.1, 1.0):
        with pytest.raises(ValueError):
            fixed.min_weight_of(outside)
...
def test_a_q12_number_clamps_to_the_port_that_carries_it():
    ...
    assert list(fixed.fixed_q12([1.0, -0.5, 0.0], 32767)) == [4096, -2048, 0]
    assert list(fixed.fixed_q12([100.0, -100.0], 32767)) == [32767, -32767]
    assert list(fixed.fixed_q12([100.0, -100.0], 131071)) == [131071, -131071]
...
def test_the_pick_lands_on_a_class_that_holds_weight():
    ...
    assert list(fixed.pick(weights, np.array([0, 0]))) == [1, 0]
    assert list(fixed.pick(weights, top)) == [2, 3]
    assert list(fixed.pick(weights, np.array([1 << 23, 1 << 23]))) == [1, 3]
