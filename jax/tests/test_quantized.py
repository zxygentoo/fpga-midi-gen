...
`nn.py` holds the part of the integer arithmetic that is one thing across the eras, and
...
def test_the_exponent_rule_holds_at_its_boundaries(peak, exponent):
    ...
    assert nn.max_exponent(peak) == exponent
...
def test_the_byte_is_symmetric_and_ties_round_up():
    ...
    q, e = nn.quantize(np.array([0.02, -0.01, 0.0]))
    assert (list(q), e) == ([82, -41, 0], 12)
    assert nn.round_half_up([-5.5, -2.5, 2.5, 5.5]).tolist() == [-5, -2, 3, 6]
...
def test_a_stated_exponent_overrides_the_tensors_own_peak():
    ...
    assert list(nn.quantize(np.array([0.02, -0.01, 0.0]), e=10)[0]) == [20, -10, 0]
    assert list(nn.quantize(np.array([1.0, -1.0]), e=14)[0]) == [127, -127]
...
def test_the_exp2_table_is_the_shared_table():
    ...
    assert nn.exp2_of_magnitude(np.int64(4096)) == 16384
...
def test_the_temper_is_log2e_over_the_temperature():
    ...
    assert nn.temper_of(1.0) == (23637, 14)
    assert nn.temper_of(0.5) == (47274, 14)
    with pytest.raises(ValueError):
        nn.temper_of(0.0)


def test_the_min_p_floor_is_a_share_of_the_peak_weight():
    """The peak weighs 2^15 after the temper, thus the floor is a plain share of it and the
    circuit compares two integers. The elected 0.05 of the frozen eras is 1638."""
    assert nn.min_weight_of(0.05) == 1638
    assert nn.min_weight_of(0.0) == 0
    for outside in (-0.1, 1.0):
        with pytest.raises(ValueError):
            nn.min_weight_of(outside)


def test_a_q12_number_clamps_to_the_port_that_carries_it():
    """Era five's per-head numbers: the `dt_bias` joins an int16 sum and the `d_skip` rides
    an 18-bit operand port, thus the bound is a fact of the CIRCUIT and the caller states
    it. A value the port cannot hold saturates and never wraps."""
    assert list(nn.fixed_q12([1.0, -0.5, 0.0], 32767)) == [4096, -2048, 0]
    assert list(nn.fixed_q12([100.0, -100.0], 32767)) == [32767, -32767]
    assert list(nn.fixed_q12([100.0, -100.0], 131071)) == [131071, -131071]
...
def test_the_pick_lands_on_a_class_that_holds_weight():
    ...
    assert list(nn.pick(weights, np.array([0, 0]))) == [1, 0]
    assert list(nn.pick(weights, top)) == [2, 3]
    assert list(nn.pick(weights, np.array([1 << 23, 1 << 23]))) == [1, 3]
