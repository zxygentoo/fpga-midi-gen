...
done, from `mamba/quantized/infer.py` over the same model. Neither side can pass by
agreeing with itself. The model crosses the seam as a CONTRACT FILE, and every width and
the plan travel in it, thus no flag of this module states a shape.
...
def contract(tmp_path, spelt, *, ring=8, **shape):
    ...
    driver reads"""
    twin = qmodel.Mamba.from_float(plan_of(spelt, **shape), ring=ring)
    path = tmp_path / "tiny.int8"
    twin.save(path)
    return path, qmodel.Mamba.load(path)
...
def streams(twin, seeds, steps):
    ...
    engine = qinfer.create_engine(twin, seeds)
    written = []
    ...
    def recorded(engine, classes, phase):
        rows, engine = qinfer.layer_streams(engine, classes, phase)
        written.append(rows)
        return engine
