...
def draw_params(key, *, d, layers, heads, state, expand, conv_scale):
    ...
    keys = iter(jax.random.split(key, len(model.TABLES) + model.PER_LAYER * layers))
    params = {
    ...
    def layer():
        ...
        conv = normal(next(keys), (channels, model.CONV_TAPS), conv_scale)
    ...
    return params | {"layers": [layer() for _ in range(layers)]}
...
def make_step(dropout, clip, weight_decay):
    def step_fn(params, m, v, t, classes, phases, lr, key):
        def loss_fn(p):
            return jnp.mean(model.seat_nll(p, classes, phases, dropout=dropout, key=key))
...
def main(
    ...
    step_fn = make_step(dropout, clip, wd)
    eval_fn = make_eval()
    ...
    click.echo(f"shape: {model.shape_of(params)}, dropout {dropout}, seed {seed}")
