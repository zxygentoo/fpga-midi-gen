...
# ---------------------------------------------------------------------
# the head over a parameter dictionary: ERA FIVE'S READER, until it takes the module
# ---------------------------------------------------------------------

# THE RULE IS `Head`'S AND IT IS STATED THERE. These four are the same arithmetic over a
# dictionary of tensors instead of a module tree, and they go the moment `mamba/model.py`
# takes the Flax shape. Their readers are `mamba/model.py`, `mamba/infer.py` and
# `mamba/quantized.py`; nothing else reads them.


def embed(params, classes):
    """`Head.embed` without the phase row, which era five's trunk adds itself"""
    return sum(params["seats"][seat][classes[..., seat]] for seat in range(data.SEATS))


def seat_logits(params, h, drawn):
    """`Head.logits` over a parameter dictionary"""
    seats = params["seats"]
    stream = h
    logits = [None] * data.SEATS
    for seat in reversed(range(data.SEATS)):
