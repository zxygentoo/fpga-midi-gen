(** The sheet: the piece as it stands, the mask over it, and the two faces that read them.
    ...
    Part of L4. One sheet is [steps] steps of [Frame.voices] seats, each cell a class of
    ...
    **THREE FACES, ONE MEMORY, AND THAT IS WHY THIS INTERFACE IS THE WIDEST OF THE
    ...
    sheet out through a port and decode it outside. The faces stand together because the
    memory does.
    ...
    - **The unit takes no clear, because the memories take no clear.** The opening writes
      ...
      cleared sheet would be a sheet no walk ever sees. The board's reset runs an opening.
    - **The score port is deliberately its own face.** Phase II gives the playing sheet
...
module Make (Shape : Shape) : sig
  ...
  (** [create i] is the block. It takes no model and no constant: a sheet holds classes,
