(* Era six's fork of [Mgen_nn.Exp2] — see exp2.mli for the contract and for the backport
   question. What changes is two registers; what does not is the table and its rules. *)
...
let create (i : _ I.t) : _ O.t =
  ...
  let integer = reg spec (select i.nn ~high:15 ~low:12) in
  let big = reg spec (select i.nn ~high:21 ~low:16 <>:. 0) in
...
let%expect_test "the fork answers what the shared unit answers" =
  (* THE EVIDENCE FOR THE BACKPORT. Driven the shared unit's way — a magnitude held for
     two cycles — the fork states the same weight on the same cycle, and both state the
     software rule. That is what makes the change safe for era four and era five, and it
