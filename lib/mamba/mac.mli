(** The MAC and its walk: the multiplier pipe of the source, one term a cycle.
    ...
    This is era four's [Mac] with wider counters, and that is the whole of the difference.
    [walk_bits] states why the import of that unit had to be a copy.
...
(** The width of the walk counters: era four's unit is this one at nine bits, and the
    state update of this machine walks 2 048 rows where era four's longest walk ran 256.
    It is the one number that made the import of that unit a copy rather than a
    dependency. *)
val walk_bits : int
