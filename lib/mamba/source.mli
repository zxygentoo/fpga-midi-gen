(** The state-space model as a circuit: the note source of era five.
    ...
    datapath: the RAMs, the state RAM, the tap rings, the banked weight ROM, and [Mac]
    behind the one multiplier. L2 is the schedule, a value: the forward pass and the chain
    of the four seats are lists of operations. L3 compiles those lists into cases of a
    program counter. L4 is a small outer machine that holds the drawn frame and answers
    the socket. source.ml holds the design and its reasons, and [docs/mamba_rtl.md] holds
    ...
    - Cycle counts are not a contract. The socket is latency-insensitive, and the PRNG
      ...
      bench pins the model to the circuit. **That cost is a constant of the shape**: era
      four's took the fill of its ring and grew with the walk; a recurrence holds a state
      of one size for ever.
    - There is one reset, and it is [rewind]: it loads the PRNG and returns the walk to
      its origin. It runs no pass, thus [idle] never falls for it. The state RAM and the
      tap ring are NOT cleared, and they do not need to be: at step 0 the state reads as
      zero through one mux and a tap reads zero by its age rule. *)
