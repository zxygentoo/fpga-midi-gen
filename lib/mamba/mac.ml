(* The MAC and its walk — see mac.mli for the contract. The datapath registers have no
   ...
   IT IS ERA FOUR'S UNIT WITH WIDER COUNTERS, and that is the whole of the difference. The
   design document of this era listed [Mac] among the units it would import as they stand,
   and the import failed on one number: era four's walks never ran past 256 rows, thus its
   counters are nine bits, and the state update of this machine walks [d_in * state] rows
   — 2 048 at the baseline shape, and 8 192 if the state sweep the model document asks for
   ever reaches 64. Fourteen bits carry both. The claim that those units are model-free is
   therefore ALMOST true: their interfaces speak widths and strobes, and the walk length
   is the one place a bigger model shows through. A round that gives the units a common
   home should make this a parameter; a prototype copies the file and says so. *)

open Base
open Hardcaml
open Signal

(* the counters of a walk: [d_in * state] rows at the baseline, and room for the state
   sweep of docs/mamba.md *)
let walk_bits = 14
let read_latency = 2
